...
- **Mining insight (miner mode)** — a compact card showing how much you've mined recently, how many rewards are still maturing, and your **holder-reward keep rate**: whether you're keeping the full mining reward or burning part of it by not holding enough KRX, plus how much more to hold to reach the next bracket. The keep rate uses the node's exact ratio-reward formula (production is estimated from recent coinbase rewards).
